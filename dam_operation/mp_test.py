...
if __name__ == "__main__":
    ...
    while t0 < te-60:
        ...
        if flow['8']>31.80/2 or flow['26']>30.40/2 or flow['62']>27.4/2 or flow['89']>16.33/2 or flow['170']>16.33/2:
            ...
            while generation<15: # generation
                ...
                arguments = [[SSN5, dam_state, t0, forcing, dam_params256, 1500] for dam_state in population]
            
                with pool as p:
                    results = p.map(RunSimulation, arguments) 
                
                end= time.time()
                print(f'time {t0}, generation: {generation} done in {end -start} seconds ')
    ...
    # path = 'C:/Users/gurbuz/Desktop/Supp_DamStudy/ga_results/'

    # s_name = '_h45_5_O75_1_10_19dams' 
    # pltkwargs = np.array([{'label':'nodam', 'color':'#1AFF1A'}, {'label':'passive', 'color':'#000000',}, 
    #                     {'label':'GA', 'color':'magenta','alpha':1.0, 'linestyle':'dashed', 'linewidth':3}])
